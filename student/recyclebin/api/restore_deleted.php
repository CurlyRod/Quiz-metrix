...
try {
    ...
    // Restore based on item type with user verification
    if ($item_type === 'quiz') {
        $stmt = $conn->prepare("UPDATE quizzes SET is_deleted = 0 WHERE quiz_id = ? AND user_id = ?");
        ...
        $stmt = $conn->prepare("UPDATE decks SET is_deleted = 0 WHERE deck_id = ? AND user_id = ?");
        $stmt->bind_param("ii", $item_id, $user_id);
    } else {
...
// Clear all output buffers and send JSON
while (ob_get_level()) {
    ob_end_clean();
}
